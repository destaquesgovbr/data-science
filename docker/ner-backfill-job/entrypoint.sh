#!/bin/sh
# Entrypoint do Cloud Run Job de backfill NER do acervo.
#
...
# discretos (boto3 os lê do ambiente), e então `exec`uta o driver de backfill NER
# repassando os args do Job ("$@", ex.: --limit/--order/--workers).
#
...
exec "$PYBIN" scripts/backfill_ner_corpus.py "$@"
